...
* New `content_document_file()` and `content_document_url()` prepare plain text, Markdown, CSV, code, and (on `ChatOpenAI()`/`ChatOpenAICompletions()`) binary office files like `.docx`/`.xlsx`/`.doc`/`.xls`/`.rtf`/`.odt` for chat input, returning a new `ContentDocument` type. Previously the only way to attach a non-PDF file was to read and string-interpolate it yourself, which lost the filename, Anthropic's citation eligibility, and OpenAI's spreadsheet parsing. Not every provider accepts every document type -- Anthropic only accepts documents it can treat as plain text (binary formats raise a clear error), and Google doesn't accept the binary office formats at all. Like `content_pdf_url()`, `content_document_url()` doesn't download up front: `ChatOpenAI()` references the URL directly, and other providers fetch lazily.
