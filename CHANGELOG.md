...
* New `content_document_file()` prepares plain text, Markdown, CSV, code, and (on `ChatOpenAI()`/`ChatOpenAICompletions()`) `.docx`/`.xlsx` files for chat input, returning a new `ContentDocument` type. Previously the only way to attach a non-PDF file was to read and string-interpolate it yourself, which lost the filename, Anthropic's citation eligibility, and OpenAI's spreadsheet parsing. Not every provider accepts every document type -- Anthropic only accepts documents it can treat as plain text (binary formats like `.docx`/`.xlsx` raise a clear error), and Google doesn't accept `.docx`/`.xlsx` at all.
