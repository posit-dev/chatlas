...
from typing import Any, Generic, Literal, Optional, TypedDict, Union
...
from ._provider import BatchStatus, BatchT
...
class BatchState(BaseModel):
    version: int
    stage: BatchStage
    batch: BaseModel | None
    results: list[BaseModel] | None
    started_at: int
    hash: BatchStateHash


class BatchJob(Generic[BatchT]):
    """
    ...
    def __init__(
        ...
        for prompt in prompts:
            if isinstance(prompt, str):
                self.user_turns.append(user_turn(prompt))
            else:
        ...
        self.batch: BaseModel | None = None
        self.results: list[BaseModel] | None = None
    ...
    def _load_state(self) -> None:
        with open(self.path, "r") as f:
            state = BatchState.model_validate(json.load(f))
