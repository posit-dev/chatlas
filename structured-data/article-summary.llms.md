...
The following example, which [closely inspired by the Claude documentation](https://github.com/anthropics/anthropic-cookbook/blob/main/tool_use/extracting_structured_json.ipynb), shows how `.chat_structured()` can be used to summarize an article.
...
chat = ChatOpenAI()
res = chat.chat_structured(text, data_model=ArticleSummary)
