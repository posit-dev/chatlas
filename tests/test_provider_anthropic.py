...
def test_anthropic_stream_merge_parses_server_tool_use_input():
    """server_tool_use input should be parsed from JSON string during streaming."""
    provider = AnthropicProvider(
        model="claude-haiku-4-5-20251001",
        api_key="test-key",
    )

    # Simulate the streaming chunks for a server_tool_use block
    ...
    # Simulate content_block_start with server_tool_use
    from unittest.mock import MagicMock

    # content_block_start
    chunk_start = MagicMock()
    chunk_start.type = "content_block_start"
    chunk_start.index = 0
    # server_tool_use block with input as empty dict initially
    block = MagicMock()
    block.type = "server_tool_use"
    block.id = "srvtoolu_1"
    block.name = "web_search"
    block.input = {}
    chunk_start.content_block = block

    completion.content.append(block)

    # content_block_delta with input_json_delta
    chunk_delta = MagicMock()
    chunk_delta.type = "content_block_delta"
    chunk_delta.index = 0
    chunk_delta.delta = MagicMock()
    chunk_delta.delta.type = "input_json_delta"
    chunk_delta.delta.partial_json = '{"query":"test search"}'

    # Need to make the content block's input a string to simulate accumulation
    completion.content[0].input = ""
    completion = provider.stream_merge_chunks(completion, chunk_delta)

    # content_block_stop should parse the JSON
    chunk_stop = MagicMock()
    chunk_stop.type = "content_block_stop"
    chunk_stop.index = 0
    completion = provider.stream_merge_chunks(completion, chunk_stop)

    # After content_block_stop, input should be parsed from JSON string to dict
    assert isinstance(completion.content[0].input, dict)
    assert completion.content[0].input == {"query": "test search"}
