...
from chatlas._tools import Tool, ToolRejectError
...
class TestNewToolConstructor:
    ...
    def test_tool_constructor_with_schema(self):
        ...
        assert tool.schema["function"]["name"] == "my_tool"
        assert tool.schema["function"]["description"] == "A test tool"
        assert tool.schema["function"]["parameters"] == parameters
...
class TestToolFromFunc:
    ...
    def test_from_func_with_model(self):
        ...
        assert tool.func == add
        assert tool.schema["function"]["name"] == "AddParams"
        assert tool.schema["function"]["description"] == "Parameters for adding numbers."
        ...
        assert params["properties"]["y"]["description"] == "Second number"
    ...
    def test_from_func_async(self):
        ...
        assert tool._is_async is True
        assert tool.schema["function"]["description"] == "Add two numbers asynchronously."
...
class TestRegisterToolForce:
    ...
    def test_register_tool_duplicate_name_error(self):
        ...
        with pytest.raises(ValueError, match="Tool with name 'add' is already registered"):
    ...
    def test_register_tool_force_overwrite(self):
        ...
        assert original_tool.schema["function"]["description"] == "Original add function."
        
        # Overwrite with force=True
        chat.register_tool(new_add, force=True)
        new_tool = chat._tools["add"]
        assert new_tool.schema["function"]["description"] == "New add function."
    ...
    def test_register_tool_with_same_name_different_function(self):
        ...
        with pytest.raises(ValueError, match="Tool with name 'test_tool' is already registered"):
