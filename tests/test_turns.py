...
def test_add_turn_system_role_error():
    ...
    with pytest.raises(ValueError, match="Turns with the role 'system' are not allowed"):
...
def test_set_turns_functionality():
    ...
    sys_msg = Turn("system", "foo")
    with pytest.raises(ValueError, match="Turn 0 has a role 'system', which is not allowed"):
