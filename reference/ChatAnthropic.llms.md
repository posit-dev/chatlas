...
| reasoning | [Optional](https://docs.python.org/3/library/typing.html#typing.Optional)\['int \| ReasoningEffort \| ThinkingConfigEnabledParam'\] | Controls Claude’s extended thinking. Pass an integer to set a fixed thinking budget (the number of tokens allocated to reasoning; must be ≥1024 and less than `max_tokens`). Alternatively, pass a string effort level (`"low"`, `"medium"`, `"high"`, `"xhigh"`, or `"max"`) to enable adaptive thinking, where Claude decides how much to think based on the requested effort. See [extended thinking](https://docs.claude.com/en/docs/build-with-claude/extended-thinking) for details. | `None` |
