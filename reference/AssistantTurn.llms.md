...
| finish_reason | [Optional](https://docs.python.org/3/library/typing.html#typing.Optional)\[`FinishReason` \| [str](https://docs.python.org/3/library/stdtypes.html#str)\] | The (normalized) reason why the turn ended, e.g. `"success"`, `"tool_use"`, `"max_tokens"`, `"content_filter"`, `"context_window"`, or `"stop_sequence"`. Providers may also surface a reason chatlas doesn’t yet recognize, in which case the raw string is passed through unchanged. | `None` |
