...
One of the most interesting aspects of modern chat models is their ability to make use of external tools provided by the caller.
Models with tool calling capabilities are [also known as "agentic" models](https://simonwillison.net/2025/May/22/tools-in-a-loop/), because they can make decisions about when and how to use these tools to achieve a specific goal or answer a question.
Agentic models are becoming the standard way to address LLM's inherent limitations, such as performing precise calculations or lacking important context[^1].
