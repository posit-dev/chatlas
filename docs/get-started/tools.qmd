...
│   Releases · tidyverse/ggplot2                                github.com │
│   … 6 more                                                               │
╰──────────────────────────────────────────────────────────────────────────╯
```

The `❝` marks a source the answer actually cites. The source list is capped at four rows -- a hard limit, and cited sources always win a slot ahead of uncited ones, so a cited source is never crowded out by search noise. Tune the cap with [`.set_echo_options()`](../reference/Chat.qmd#set_echo_options) -- pass `web_activity_max_sources=None` to list every source instead.
...
How much of this you see is controlled by the `echo` parameter, same as for [reasoning](reasoning.qmd#seeing-reasoning): the default (`echo="output"`) includes it; `echo="text"` shows just the model's answer.
