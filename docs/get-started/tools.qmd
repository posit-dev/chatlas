...
`.stream()` doesn't automatically include tool calls information; however, you can set `content="all"` to receive all content, including tool calls.
In later articles, you'll learn more about [customizing tool call displays](../tool-calling/displays.qmd) and also adding a [tool approval mechanism](../tool-calling/approval.qmd).
