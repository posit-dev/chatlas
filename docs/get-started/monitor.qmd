...
Framework spans complement the provider-specific instrumentors described below.
Because chatlas activates its spans in the OpenTelemetry context, spans created
by other libraries nest underneath automatically:

- A provider instrumentor's **HTTP span** appears under the relevant `chat` span.
- Any spans your **tools** emit (e.g. a database query or outbound HTTP call)
  appear under that tool's `execute_tool` span.

The result is one connected trace spanning the agent loop, each model call, each
tool execution, and the underlying HTTP detail.
