...
This makes it a better choice than [`.chat()`](../reference/Chat.qmd#chat) for programming bespoke experiences such as [chatbot](chatbot-apps.qmd) apps, where some other framework is consuming/displaying the response.
...
Similar to `.chat()`, once the model is done responding, the user and assistant turns are stored on the `Chat` instance:
...
As we'll learn later, this can be useful for customizing how tool calls are handled and displayed in your app.
