...
For a large file, or one you want to reference across many turns, upload it once with [`chat.files.upload()`](../reference/FileManager.qmd#upload) instead of re-sending its bytes every time:
...
See [`chat.files`](../reference/FileManager.qmd) for the full set of upload/list/download/delete operations and provider support notes, and [`ContentUploaded`](../reference/types.ContentUploaded.qmd) for the reference type it returns.
