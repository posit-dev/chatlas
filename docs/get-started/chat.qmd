...
Of the major providers, only `ChatGoogle()`/`ChatVertex()` (Gemini) accepts video. For a small local clip, use [`content_video_file()`](../reference/content_video_file.qmd); for anything larger, prefer `chat.files.upload()` above. A public YouTube video can be referenced directly, with no upload at all:
...
This is a free preview feature limited to public videos, up to 10 per request on Gemini 2.5+ models. Passing video content to any other provider raises a clear error.
:::
