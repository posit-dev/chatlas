...
Audio input works the same way, via [`content_audio_file()`](../reference/content_audio_file.qmd):

```python
chat = ctl.ChatGoogle()
chat.chat(
  "What's being said in this recording?",
  ctl.content_audio_file("meeting.wav"),
)
```
...
Not every model supports every content type. Please refer to the documentation for the specific model you're using to see which content types are supported. Notably, audio input only works with [`ChatGoogle()`](../reference/ChatGoogle.qmd)/[`ChatVertex()`](../reference/ChatVertex.qmd) (wav, mp3, aiff, aac, ogg, flac) and [`ChatOpenAICompletions()`](../reference/ChatOpenAICompletions.qmd) (wav, mp3 only) -- Anthropic and the OpenAI Responses API (`ChatOpenAI()`) don't accept audio input at all.
:::
