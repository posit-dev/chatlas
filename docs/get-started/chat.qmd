...
`content_pdf_url()` doesn't download the PDF's bytes up front -- Anthropic and
`ChatOpenAI()` (the Responses API) can reference the URL directly, so the
bytes are only fetched if the target provider actually needs them (e.g.
`ChatGoogle()` or `ChatOpenAICompletions()`).
