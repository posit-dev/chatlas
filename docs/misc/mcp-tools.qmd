...
As a result, there are now [many useful MCP server implementations available](https://glama.ai/mcp/servers) to help extend the capabilities of your chat application.
In this article, you'll learn the basics of using and implementing MCP tools in chatlas.
...
The [awesome-mcp-servers](https://github.com/punkpeye/awesome-mcp-servers) repository maintains a comprehensive list of available MCP servers.
Common categories include:

- **Web**: Fetch URLs, search the web, browse pages
- **Filesystem**: Read/write files, search directories
- **Databases**: Query SQL databases, access data stores
- **Code execution**: Run Python, JavaScript, or other code safely
- **APIs**: Connect to GitHub, Slack, and other services

Most servers can be run via `npx` (for JavaScript-based servers) or other package managers, making them easy to integrate with chatlas.
...
Use this when running a server locally via a command. This is the most common approach for 3rd party servers.
...
Use this when connecting to an already-running HTTP server.
