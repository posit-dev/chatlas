...
However, RAG is not always necessary -- sometimes something much simpler will do the trick, such as:
  ...
  * Adding trusted/missing content directly to the [system prompt](../get-started/system-prompt.qmd) or [user query](../get-started/user-query.qmd), rather than retrieving it from a knowledge store.
...
def retrieve_trusted_content(query):
    ...
    return [f"<excerpt>{x.text}</exercpt>" for x in nodes]
```
