...
Once you’ve choosen a [model](../get-started/models.llms.md), initialize the relevant [`Chat`](../reference/Chat.llms.md) client instance. Here you access to handful of parameters, most importantly the `model` and `system_prompt`:
...
Soon we’ll learn more about the [system prompt](../get-started/system-prompt.llms.md), but for now, just know that it is the primary place for you (the developer) to influence the model’s behavior.

## Submit input

Use the [`.chat()` method](../reference/Chat.llms.md#chat) to submit input and get a streaming response echoed back to an interactive display, like a notebook or console. This echoing behavior is great for interactive prototyping, but not for other settings, such as web apps or GUIs. In the latter case, use the [`.stream()` method](../get-started/stream.llms.md) instead to get a response generator that you can consume and display however you like.
...
This means that the model is provided the entire conversation history on each new submission. This again is typically the desirable behavior, but sometimes you may want to fork, reset, condense, or otherwise [manage history](#manage-history).

## Dedicated chat

When you’re first starting out testing the capabilities of a model, repeatedly calling `.chat()` is a bit tedious. Instead, consider using the [`.console()`](../reference/Chat.llms.md#console) or [`.app()`](../reference/Chat.llms.md#app) methods to launch a dedicated chat interface. They will save you a bit of typing and, in the case of `.app()`, enables a more interactive, browser-based, experience.
...
[![Screenshot of a dedicated chat web app](../images/chat-app.png)](../images/chat-app.png "Screenshot of a dedicated chat web app")

Screenshot of a dedicated chat web app

## Console

[![Screenshot of a dedicated chat console](../images/chat-console.png)](../images/chat-console.png "Screenshot of a dedicated chat console")

Screenshot of a dedicated chat console

## Save history

Printing `chat` at the console shows the conversation history, but you can also `.export()` it to a more readable markdown or HTML file. When exporting to HTML, you’ll get a display similar to the [dedicated chat app](#dedicated-chat).
...
> **TIP:**
>
> Since [`Turn`](../reference/Turn.llms.md)s inherit from pydantic’s [`BaseModel`](https://docs.pydantic.dev/latest/usage/models/#model), you can also serialize/unserialize them to JSON, which is useful for saving/loading the history to/from a database or file.
>
> ``` python
...
The `chat` history is stored as a list of [`Turn`](../reference/Turn.llms.md) objects. To get/set them, use `.get_turns()` / `.set_turns()`.
