...
Once your workflow can handle sequential requests successfully, you may want to scale up to numerous requests at once. This is especially useful if you’re scaling up a [structured data extraction](get-started/structured-data.qmd) to process many documents at once. With chatlas, you have two options for submitting many requests at once, each with its own tradeoff:
...
[`parallel_chat()`](reference/parallel_chat.qmd) takes a `Chat` instance and a list of prompts, and submits the prompts concurrently. This is typically much faster than submitting them one at a time:
...
> **NOTE:**
>
> `parallel_chat()` (and friends) are [asynchronous](get-started/async.qmd), so you must `await` them. Notebooks (e.g., Jupyter, Positron, VS Code) support top-level `await`, so the code above works as-is. In a plain Python script, you’ll need to wrap it in an `async` function and run it with `asyncio.run()`:
>
> ``` python
> import asyncio
>
...
The result is a list of `Chat` objects, one per prompt, each containing its own conversation history. If you only care about the text of each response, use [`parallel_chat_text()`](reference/parallel_chat_text.qmd) instead:
...
If you can wait up to 24 hours for results, batch requests cost 50% less than regular requests. [`batch_chat()`](reference/batch_chat.qmd) submits prompts through a provider’s batch API (currently supported for OpenAI, Anthropic, Google (Gemini), and Groq):
